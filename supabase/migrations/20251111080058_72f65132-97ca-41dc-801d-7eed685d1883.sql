-- Fix admin_adjust_user_balance to correctly update balances
...
BEGIN
  ...
  IF p_balance_type = 'bsk' THEN
    ...
    -- Upsert with correct balance updates
    IF p_subtype = 'withdrawable' THEN
      ...
      VALUES (p_target_user_id, p_amount, 0, p_amount, 0)
    ...
    ELSE
      INSERT INTO user_bsk_balances (user_id, withdrawable_balance, holding_balance, total_earned_withdrawable, total_earned_holding)
      VALUES (p_target_user_id, 0, p_amount, 0, p_amount)
      ON CONFLICT (user_id) DO UPDATE SET
  ...
  -- Log the adjustment (this will trigger the notification)
