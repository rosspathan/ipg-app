
-- =============================================
-- #3: Add source balance verification to internal balance transfer
-- When transferring to_trading, verify and debit onchain_balances
-- When transferring from_trading, credit onchain_balances
-- =============================================
CREATE OR REPLACE FUNCTION public.execute_internal_balance_transfer(
  p_user_id UUID,
  p_asset_id UUID,
  p_amount NUMERIC,
  p_direction TEXT  -- 'to_trading' or 'from_trading'
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  v_balance RECORD;
  v_onchain RECORD;
  v_new_available NUMERIC;
  v_new_onchain NUMERIC;
  v_transfer_id UUID;
BEGIN
  -- Validate direction
  IF p_direction NOT IN ('to_trading', 'from_trading') THEN
    RETURN jsonb_build_object('success', false, 'error', 'Invalid direction');
  END IF;

  -- Validate amount
  IF p_amount <= 0 THEN
    RETURN jsonb_build_object('success', false, 'error', 'Amount must be positive');
  END IF;

  -- Round to 8 decimals
  p_amount := ROUND(p_amount, 8);

  IF p_direction = 'to_trading' THEN
    -- ===== WALLET → TRADING =====
    -- Step 1: Lock and verify onchain_balances (source)
    SELECT id, balance
      INTO v_onchain
      FROM onchain_balances
     WHERE user_id = p_user_id AND asset_id = p_asset_id
       FOR UPDATE;

    IF v_onchain IS NULL OR v_onchain.balance < p_amount THEN
      RETURN jsonb_build_object(
        'success', false,
        'error', format('Insufficient wallet balance. Available: %s', 
                        COALESCE(ROUND(v_onchain.balance, 8)::text, '0'))
      );
    END IF;

    -- Step 2: Debit onchain_balances
    v_new_onchain := ROUND(v_onchain.balance - p_amount, 8);
    UPDATE onchain_balances
       SET balance = v_new_onchain,
           last_synced_at = now()
     WHERE id = v_onchain.id;

    -- Step 3: Credit wallet_balances (trading)
    SELECT id, available, locked
      INTO v_balance
      FROM wallet_balances
     WHERE user_id = p_user_id AND asset_id = p_asset_id
       FOR UPDATE;

    IF v_balance IS NULL THEN
      INSERT INTO wallet_balances (user_id, asset_id, available, locked)
      VALUES (p_user_id, p_asset_id, p_amount, 0);
    ELSE
      v_new_available := ROUND(v_balance.available + p_amount, 8);
      UPDATE wallet_balances
         SET available = v_new_available,
             updated_at = now()
       WHERE id = v_balance.id;
    END IF;

  ELSE
    -- ===== TRADING → WALLET =====
    -- Step 1: Lock and verify wallet_balances (source)
    SELECT id, available, locked
      INTO v_balance
      FROM wallet_balances
     WHERE user_id = p_user_id AND asset_id = p_asset_id
       FOR UPDATE;

    IF v_balance IS NULL THEN
      RETURN jsonb_build_object('success', false, 'error', 'No trading balance found');
    END IF;

    IF v_balance.available < p_amount THEN
      RETURN jsonb_build_object(
        'success', false,
        'error', format('Insufficient trading balance. Available: %s', ROUND(v_balance.available, 8))
      );
    END IF;

    -- Step 2: Debit wallet_balances
    v_new_available := ROUND(v_balance.available - p_amount, 8);
    UPDATE wallet_balances
       SET available = v_new_available,
           updated_at = now()
     WHERE id = v_balance.id;

    -- Step 3: Credit onchain_balances
    SELECT id, balance
      INTO v_onchain
      FROM onchain_balances
     WHERE user_id = p_user_id AND asset_id = p_asset_id
       FOR UPDATE;

    IF v_onchain IS NULL THEN
      INSERT INTO onchain_balances (user_id, asset_id, balance, last_synced_at)
      VALUES (p_user_id, p_asset_id, p_amount, now());
    ELSE
      v_new_onchain := ROUND(v_onchain.balance + p_amount, 8);
      UPDATE onchain_balances
         SET balance = v_new_onchain,
             last_synced_at = now()
       WHERE id = v_onchain.id;
    END IF;
  END IF;

  -- Record the transfer
  INSERT INTO trading_balance_transfers (user_id, asset_id, direction, amount, status, completed_at)
  VALUES (p_user_id, p_asset_id, p_direction, p_amount, 'completed', now())
  RETURNING id INTO v_transfer_id;

  RETURN jsonb_build_object(
    'success', true,
    'transfer_id', v_transfer_id,
    'amount', p_amount,
    'direction', p_direction
  );
END;
$$;
