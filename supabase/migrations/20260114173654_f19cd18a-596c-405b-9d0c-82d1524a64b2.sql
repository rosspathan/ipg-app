-- Fix fee rate mismatch in place_order_atomic: change from 0.1% to 0.5%
-- This ensures buy orders lock: amount × price × 1.005 (matching execute_trade expectations)
...
DECLARE
  ...
  v_fee_rate NUMERIC := 0.005; -- 0.5% fee (FIXED from 0.001)
  v_order_id UUID;
