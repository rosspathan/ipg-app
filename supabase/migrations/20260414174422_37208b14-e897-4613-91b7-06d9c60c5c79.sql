...
-- Step 1: Clean up failed events and their user records
DELETE FROM public.bsk_unlock_user_records 
WHERE event_id IN (SELECT id FROM public.bsk_global_unlock_events WHERE status = 'failed');

DELETE FROM public.bsk_global_unlock_events WHERE status = 'failed';

-- Step 2: Recreate execute function with fixed reference_id cast
CREATE OR REPLACE FUNCTION public.execute_bsk_global_unlock(p_confirmation_phrase TEXT)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      INSERT INTO public.bsk_holding_ledger (
        ...
        v_event_id  -- UUID directly, no ::TEXT cast
      );
  ...
  SET status = CASE WHEN v_fail_count = 0 AND v_success_count > 0 THEN 'completed' WHEN v_success_count = 0 THEN 'failed' ELSE 'partial' END,
  ...
  -- Return failure if no users were successfully processed but there were users to process
  IF v_success_count = 0 AND v_total_locked > 0 THEN
    RETURN jsonb_build_object(
      'success', false,
      'reason', 'all_users_failed',
      'event_id', v_event_id,
      'users_failed', v_fail_count,
      'total_locked_found', v_total_locked,
      'first_error', CASE WHEN jsonb_array_length(v_errors) > 0 THEN v_errors->0->>'error' ELSE 'Unknown' END
    );
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'event_id', v_event_id,
    'status', CASE WHEN v_fail_count = 0 AND v_success_count > 0 THEN 'completed' WHEN v_success_count = 0 THEN 'failed' ELSE 'partial' END,
