-- Fix idempotency key duplication in execute_bsk_transfer
...
BEGIN
  ...
  -- Record transaction for sender (outgoing) - FIXED: unique idempotency key
  ...
  -- Record transaction for recipient (incoming) - FIXED: unique idempotency key
  INSERT INTO unified_bsk_ledger (
    user_id,
    tx_type,
    tx_subtype,
  ...
  -- Also record in unified_bsk_transactions for backward compatibility
