...
-- Configure multi-level commission structure for enabled settings
...
-- Create function to get commission rate for a level
...
-- Create retroactive commission reconciliation function
...
BEGIN
  ...
  expected_commissions AS (
    ...
    WHERE rt.level <= COALESCE(
      (SELECT max_levels FROM team_referral_settings WHERE enabled = true LIMIT 1),
      50
    )
    AND get_commission_rate_for_level(
      rt.level,
      (SELECT level_percentages FROM team_referral_settings WHERE enabled = true LIMIT 1)
    ) > 0
  ),
  actual_commissions AS (
    ...
    WHERE rc.commission_type IN ('badge_purchase', 'badge_purchase_retroactive')
  ...
  SELECT
    ...
    ec.payer_id,
    ec.level,
    ec.badge_purchase_id,
    ec.expected_amount as should_have_earned,
    COALESCE(ac.paid_amount, 0) as actually_earned,
  ...
  LEFT JOIN actual_commissions ac 
    ON ac.earner_id = ec.sponsor_id 
    AND ac.payer_id = ec.payer_id
    AND ac.level = ec.level
    AND ac.event_id = ec.badge_purchase_id
  WHERE (ec.expected_amount - COALESCE(ac.paid_amount, 0)) > 0.01
  ORDER BY ec.sponsor_id, ec.level;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO referral_commissions (
      ...
      v_rec.level,
      'badge_purchase',
      v_rec.badge_purchase_id,
      v_rec.missing_commission,
      'badge_purchase_retroactive',
      'withdrawable',
      'settled',
      COALESCE(v_badge_name, 'UNKNOWN'),
      NOW(),
      NOW()
    );
    ...
    INSERT INTO bonus_ledger (
      ...
      v_rec.missing_commission,
      'withdrawable',
      'retroactive_commission',
      v_rec.badge_purchase_id::text,
      jsonb_build_object(
        'level', v_rec.level,
        'payer_id', v_rec.payer_id,
        'commission_type', 'badge_purchase_retroactive',
        'retroactive', true
      ),
      NOW()
    );
  ...
  FROM calculate_retroactive_commissions();

  RETURN QUERY SELECT v_total_sponsors, v_total_paid, v_total_entries;
END;
$$ LANGUAGE plpgsql;

-- Add index for better performance on commission lookups
CREATE INDEX IF NOT EXISTS idx_referral_commissions_earner_payer_level 
  ON referral_commissions(earner_id, payer_id, level, commission_type);

CREATE INDEX IF NOT EXISTS idx_referral_commissions_event_id 
  ON referral_commissions(event_id);

-- Add audit table for retroactive payments
...
GRANT SELECT ON retroactive_commission_audit TO authenticated;
GRANT INSERT ON retroactive_commission_audit TO authenticated;

-- Create RLS policies for audit table
CREATE POLICY "Admin can view retroactive audit"
  ON retroactive_commission_audit
  FOR SELECT
  USING (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY "System can insert retroactive audit"
  ON retroactive_commission_audit
  FOR INSERT
  WITH CHECK (true);