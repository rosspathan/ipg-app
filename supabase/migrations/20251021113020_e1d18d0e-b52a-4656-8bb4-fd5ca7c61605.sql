-- Fix admin_adjust_user_balance function to properly handle multiple adjustments
-- The issue was that ON CONFLICT was replacing the balance instead of adding to it
...
begin
  ...
  if p_balance_type = 'bsk' then
    ...
    -- Fix: Proper upsert that adds to existing balance
    ...
    -- Get the final balance after update
    select withdrawable_balance into new_balance
    from public.user_bsk_balances
    where user_id = p_target_user_id;
    ...
    insert into public.audit_logs (
      ...
      jsonb_build_object('delta', adj_amount, 'reason', p_reason, 'operator', auth.uid(), 'new_balance', new_balance), now()
    );
...
comment on function public.admin_adjust_user_balance(uuid, text, text, numeric, text)
  is 'Admin-only RPC to adjust user BSK/INR balances and write audit logs. Fixed to properly add/deduct from existing balances.';