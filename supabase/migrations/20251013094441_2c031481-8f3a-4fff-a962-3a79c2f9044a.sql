-- Fix admin_reset_all_user_balances with correct column names
...
BEGIN
  ...
  -- Reset user_bonus_balances (has bsk_available and bsk_pending columns)
  UPDATE public.user_bonus_balances
  SET 
    bsk_available = 0,
    bsk_pending = 0,
    updated_at = NOW()
  WHERE bsk_available != 0 OR bsk_pending != 0;
  
  GET DIAGNOSTICS v_bonus_balance_records = ROW_COUNT;

  -- Reset wallet_bonus_balances (crypto bonus balances - has balance column)
  ...
  FROM (
    ...
    UNION
    SELECT user_id::uuid FROM public.user_bonus_balances WHERE user_id IS NOT NULL
    UNION
