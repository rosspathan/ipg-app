...
-- ============================================================
-- FIX 3: ATOMIC WITHDRAWAL RECORD CREATION INSIDE RPC
-- Extend execute_internal_balance_transfer to insert into 'withdrawals'
-- within the same atomic transaction for from_trading direction
-- ============================================================

CREATE OR REPLACE FUNCTION public.execute_internal_balance_transfer(
  p_user_id uuid, p_asset_id uuid, p_amount numeric, p_direction text, 
...
BEGIN
  ...
  SELECT symbol, network INTO v_asset_symbol, v_asset_network FROM assets WHERE id = p_asset_id;
  ...
  ELSE
    ...
    -- *** FIX 3: ATOMIC WITHDRAWAL RECORD CREATION ***
    -- Create the withdrawal record INSIDE this transaction so if anything fails,
    -- the entire operation (debit + ledger + withdrawal) is rolled back together.
    IF p_to_address IS NOT NULL AND p_to_address != '' THEN
      INSERT INTO withdrawals (
        user_id, asset_id, amount, fee, net_amount, 
        to_address, network, status
      ) VALUES (
        p_user_id, p_asset_id, p_amount, 0, p_amount,
        p_to_address, COALESCE(v_asset_network, 'BEP20'), 'processing'
      )
      RETURNING id INTO v_withdrawal_id;

      -- Link withdrawal to the transfer
      UPDATE internal_balance_transfers
      SET notes = format('withdrawal_id:%s', v_withdrawal_id)
...
-- Drop the old 5-parameter version to prevent ambiguity
DROP FUNCTION IF EXISTS public.execute_internal_balance_transfer(uuid, uuid, numeric, text, text);
