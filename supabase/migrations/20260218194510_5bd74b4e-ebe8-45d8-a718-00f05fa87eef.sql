...
-- Add a proper unique constraint on symbol (the index exists but not as a constraint)
-- First check if we can add it (the unique index already exists, so we just need to convert it)
ALTER TABLE market_prices ADD CONSTRAINT market_prices_symbol_key UNIQUE USING INDEX market_prices_symbol_unique;

-- Now fix execute_trade to handle the market_prices upsert correctly
...
BEGIN
  ...
  -- Update market price using direct UPDATE (avoids INSERT conflict with market_id NOT NULL)
  UPDATE market_prices
  SET 
    current_price = p_quote_amount / v_fill_quantity,
    last_updated = NOW(),
    updated_at = NOW()
  ...
  IF NOT FOUND THEN
    INSERT INTO market_prices (symbol, current_price, market_id, last_updated)
    SELECT p_symbol, p_quote_amount / v_fill_quantity, m.id, NOW()
