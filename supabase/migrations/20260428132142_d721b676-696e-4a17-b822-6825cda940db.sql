-- Admin-only KYC diagnostics: per-user, per-action verdict
-- Returns one JSON document so the admin UI can render a single panel with no extra round-trips.
...
BEGIN
  ...
  -- Single human-readable reason for the whole gate
  IF v_approved THEN
    v_block_reason := NULL;
  ...
  -- All 10 sensitive actions share the same gate today (is_kyc_approved).
  -- We still emit a per-action map so the UI can show explicit ✓ / ✗ rows
  -- and so future per-action toggles (e.g. system_settings) can be wired here.
  ...
  RETURN jsonb_build_object(
    ...
    'pillars', jsonb_build_object(
      ...
      'mobile', jsonb_build_object(
        ...
        'reviewed_at', v_profile.mobile_reviewed_at,
        'reviewed_by', v_profile.mobile_reviewed_by,
...
REVOKE ALL ON FUNCTION public.admin_kyc_access_check(uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.admin_kyc_access_check(uuid) TO authenticated, service_role;

COMMENT ON FUNCTION public.admin_kyc_access_check(uuid) IS
'Admin-only diagnostics. Returns the 4 KYC pillar statuses + per-sensitive-action allowed/blocked verdict for one user. Auth check inside body (has_role admin). Used by Admin KYC Diagnostics panel.';
