...
BEGIN
  ...
  ON CONFLICT (user_id, asset_id) DO UPDATE SET
    available = wallet_balances.available + v_fill_quantity, updated_at = NOW();
  ...
  ON CONFLICT (user_id, asset_id) DO UPDATE SET
    available = wallet_balances.available + (p_quote_amount - p_seller_fee), updated_at = NOW();
  
  INSERT INTO wallet_balances (user_id, asset_id, available, locked)
  VALUES (v_platform_account_id, v_quote_asset_id, v_total_fees, 0)
  ON CONFLICT (user_id, asset_id) DO UPDATE SET
    available = wallet_balances.available + v_total_fees, updated_at = NOW();
  
  -- ===== UPDATE ORDERS =====
  UPDATE orders SET
    filled_amount = v_new_buy_filled, status = v_buy_new_status,
    locked_amount = v_buy_remaining_lock,
    average_price = CASE WHEN v_new_buy_filled > 0 THEN
      (COALESCE(average_price,0) * COALESCE(v_buy_order.filled_amount,0) + v_exec_price * v_fill_quantity) / v_new_buy_filled
  ...
  UPDATE orders SET
    filled_amount = v_new_sell_filled, status = v_sell_new_status,
    locked_amount = v_sell_remaining_lock,
    average_price = CASE WHEN v_new_sell_filled > 0 THEN
      (COALESCE(average_price,0) * COALESCE(v_sell_order.filled_amount,0) + v_exec_price * v_fill_quantity) / v_new_sell_filled
      ELSE v_exec_price END,
  ...
  INSERT INTO trades (
    ...
    v_exec_price, v_fill_quantity, p_buyer_fee, p_seller_fee, p_quote_amount, NOW(), p_trading_type
  );
  
  -- ===== UPDATE MARKET PRICE (use UPDATE only, row must already exist) =====
  -- Look up existing market_id for this symbol
  SELECT market_id INTO v_market_id FROM market_prices WHERE symbol = p_symbol;
  
  IF v_market_id IS NOT NULL THEN
    -- Row exists, just update price
    UPDATE market_prices SET
      current_price = v_exec_price,
      last_updated = NOW(),
      updated_at = NOW()
    WHERE symbol = p_symbol;
  ELSE
    -- No market_prices row for this symbol - skip price update rather than fail the trade
    RAISE WARNING 'No market_prices row for symbol %, skipping price update', p_symbol;
  END IF;
