...
-- Fix the credit_custodial_deposit function's ON CONFLICT to match the actual unique index
-- The unique index is: (reference_type, reference_id, entry_type, user_id) WHERE reference_id IS NOT NULL
-- But the RPC uses ON CONFLICT (reference_type, reference_id) which doesn't match
...
BEGIN
  ...
  ELSE
    INSERT INTO public.wallet_balances (
      user_id, asset_id, available, locked, created_at, updated_at
    ) VALUES (
      v_dep.user_id, v_dep.asset_id, v_dep.amount, 0, now(), now()
    )
    RETURNING available, locked INTO v_new_available, v_new_locked;
  END IF;

  -- Ledger entry (idempotent via the actual unique index on reference_type, reference_id, entry_type, user_id)
