...
export default function GenerateHotWallet() {
  ...
  // Fetch current trading hot wallet - scoped by label
  const { data: tradingWallet, isLoading } = useQuery({
    queryKey: ['trading-hot-wallet'],
    queryFn: async () => {
  ...
  const { data: walletBalance } = useQuery({
    ...
    queryFn: async () => {
      if (!tradingWallet?.address) return null;
      try {
        const provider = new ethers.JsonRpcProvider('https://bsc-dataseed1.binance.org/');
        const balance = await provider.getBalance(tradingWallet.address);
        return {
          bnb: ethers.formatEther(balance),
          isLowGas: parseFloat(ethers.formatEther(balance)) < 0.1
        };
      } catch (error) {
        return { bnb: '0', isLowGas: true };
      }
    },
    enabled: !!tradingWallet?.address
  });
    ...
    <div className="min-h-screen bg-background p-4 md:p-8">
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={() => navigate('/admin/nova')}>
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              View-only — Custodial exchange trading wallet
            </p>
          </div>
        </div>
        ...
        {tradingWallet ? (
          <Card className={walletBalance?.isLowGas ? "border-destructive" : "border-green-500/50"}>
            <CardHeader>
              ...
              <CardDescription>Current trading hot wallet status</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label>Wallet Address</Label>
                <div className="flex gap-2">
                  <Input value={tradingWallet.address} readOnly className="font-mono text-sm" />
                  <Button variant="outline" size="icon" onClick={() => copyToClipboard(tradingWallet.address, 'Address')}>
              ...
              {walletBalance?.isLowGas && (
                <Alert variant="destructive">
                  ...
                  <AlertDescription>
                    Send BNB to <code className="bg-muted px-1 rounded">{tradingWallet.address}</code> to ensure withdrawals can be processed.
                  </AlertDescription>
                </Alert>
              )}

              <a
                href={`https://bscscan.com/address/${tradingWallet.address}`}
                target="_blank"
          ...
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <h3 className="text-lg font-semibold mb-2">No Trading Wallet Configured</h3>
              <p className="text-muted-foreground text-sm">
                Contact the system administrator to provision a trading wallet via database migration.
              </p>
            </CardContent>
          </Card>
        )}
        ...
        <Card>
          ...
          <CardContent>
            <ul className="text-sm text-muted-foreground space-y-2">
              ...
              <li>• <strong>Migration Wallet</strong> — Dedicated to BSK on-chain migration transfers</li>
