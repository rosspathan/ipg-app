...
export default function StakingHotWalletAdmin() {
  ...
  // Fetch current staking wallet config
  const { data: stakingConfig, isLoading: configLoading } = useQuery({
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              View-only — Dedicated wallet for IPG staking deposits
            </p>
          </div>
        </div>
        ...
        {stakingConfig?.admin_hot_wallet_address ? (
          <Card className={walletBalance?.isLowGas ? "border-destructive" : "border-green-500/50"}>
            <CardHeader>
              ...
              <CardDescription>Current staking hot wallet status</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label>Wallet Address</Label>
                <div className="flex gap-2">
                  <Input value={stakingConfig.admin_hot_wallet_address} readOnly className="font-mono text-sm" />
                  <Button variant="outline" size="icon" onClick={() => copyToClipboard(stakingConfig.admin_hot_wallet_address, 'Address')}>
          ...
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <h3 className="text-lg font-semibold mb-2">No Staking Wallet Configured</h3>
              <p className="text-muted-foreground text-sm">
                Contact the system administrator to provision a staking wallet via database migration.
              </p>
            </CardContent>
          </Card>
        )}
        ...
        <Card>
          ...
          <CardContent>
            <ul className="text-sm text-muted-foreground space-y-2">
              <li>• <strong>Staking Wallet</strong> — Only accepts IPG token deposits for staking</li>
