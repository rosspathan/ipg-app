...
import { AdminHeader } from '@/components/navigation/NavigationHeader';
...
// Admin route titles mapping
const ADMIN_ROUTE_TITLES: Record<string, string> = {
  '/admin': 'Admin Dashboard',
  '/admin/dashboard': 'Admin Dashboard',
  '/admin/users': 'User Management',
  '/admin/assets': 'Assets & Tokens',
  '/admin/markets': 'Markets Management',
  '/admin/fees': 'Fees & Revenue',
  '/admin/funding': 'Funding Management',
  '/admin/funding/inr': 'INR Funding',
  ...
  '/admin/spin': 'Spin Wheel Management',
  '/admin/purchase-bonus': 'Purchase Bonus',
  '/admin/insurance': 'Insurance Management',
  '/admin/insurance/claims': 'Insurance Claims',
  '/admin/system': 'System Settings',
  '/admin/test': 'Credentials Test'
};
...
const AdminLayout = () => {
  ...
  const getPageTitle = (): string => {
    ...
    if (ADMIN_ROUTE_TITLES[path]) {
      return ADMIN_ROUTE_TITLES[path];
    }
    
    // Check for dynamic routes (e.g., /admin/markets/BTC-USDT)
    const basePath = path.split('/').slice(0, 3).join('/'); // Get /admin/section
    if (ADMIN_ROUTE_TITLES[basePath]) {
      return ADMIN_ROUTE_TITLES[basePath];
    }
  ...
  const isRootAdmin = location.pathname === '/admin' || location.pathname === '/admin/dashboard';
  ...
  return (
    <NavigationStateManager>
      <div className="min-h-screen bg-background">
        ...
        <AdminHeader 
          title={getPageTitle()}
          showBack={!isRootAdmin}
          showMenu={isRootAdmin}
        />
        
        {/* Top Header - Keep existing for now */}
