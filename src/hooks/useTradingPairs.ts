...
export function useTradingPairs(type?: 'listed' | 'all') {
  ...
  // Realtime: new/updated tokens or markets from Admin → Token Management appear instantly
  useEffect(() => {
    const channel = supabase
      .channel('trading-pairs-catalog')
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Fetch latest prices from market_prices (public table - same for all users)
      // This ensures all users see the same price, not user-scoped trade data
