...
export const useProfile = () => {
  ...
  const fetchUserApp = async () => {
    ...
    try {
      ...
      if (!data) {
        // Create initial user app record (referral_code will be auto-generated by trigger)
        const { data: newUserApp, error: createError } = await supabase
          ...
          .insert([{
            ...
            referral_code: user.id.substring(0, 8).toUpperCase() // Temporary code, will be replaced if trigger runs
          }])
          .select()
          .single();
